...
class Feature:
    ...
    def supported_update_fields() -> []:
        ...
        def generic_feature_update(values_to_update: [str], params: tuple, db):
            db.executeQuery(
                f"UPDATE feature SET {db.update_set_helper(
                    values_to_update)} WHERE feature_id=%s AND parent_service=%s",
                paramatized=params
            )
        ...
        def get_all_features(db):
            ...
            for feature in get_features:
                featureObjs.append(
                    Feature(feature[0], feature[1], feature[2], feature[3], feature[4], feature[5], feature[6]))
        ...
        def new_feature(name, description, parent, db):
            ...
            if get_feature_by_name(parent, name, db) is None:
        ...
        def deactivate_feature(service_id, id, db):
            if get_feature_by_id(id, db) is not None:
